 ...
 * 
 * This is meant to run on a SEGA Lindbergh
 * 
 * Author: bobbydilley
...
int processInputPacket(char *packet, RideState *state)
{
	...
	state->PlayerOneBlowFront = (packet[3] & 1) > 0;
	state->PlayerTwoBlowFront = (packet[3] & 2) > 0;
	state->PlayerOneBlowBack = (packet[3] & 4) > 0;
	state->PlayerTwoBlowBack = (packet[3] & 8) > 0;
	state->PlayerOneGunReaction = (packet[3] & 16) > 0;
	state->PlayerTwoGunReaction = (packet[3] & 32) > 0;
	...
	/* Other Lamp outputs */
	state->ResetLamp = (packet[5] & 1) > 0;
	state->ErrorLamp = (packet[5] & 2) > 0;
	state->SafetyLamp = (packet[5] & 4) > 0;
	state->GameStopLamp = (packet[5] & 8) > 0;
	state->FloorLamp = (packet[5] & 16) > 0;
	state->SpotLamp = (packet[5] & 32) > 0;
...
int processOutputPacket(char *packet, RideState *state)
{
	...
	if (state->InitButton)
		packet[4] = packet[4] | 16;

	if (state->RearFootSensor)
		packet[4] = packet[4] | 32;

	if (state->LeftFootSensor)
		packet[4] = packet[4] | 64;
	...
	char checksum = 0;
	int i;
	for (i = 1; i < OUTPUT_PACKET_SIZE - 1; i++)
		checksum ^= packet[i];
...
int main(int argc, char **argv)
{
	...
	printf("Info: Closing + Removing FIFO\n");
